...
void Monitor::updateENS(uint8_t aqi, uint16_t tvoc, uint16_t eco2) {
    printENSDisplay(aqi, tvoc, eco2);
}
...
void Monitor::printENSDisplay(uint8_t aqi, uint16_t tvoc, uint16_t eco2) {
    // Τελευταία γραμμή για ποιότητα αέρα
    display.setTextColor(ST7735_GREEN, ST7735_BLACK);
    if (aqi > 2) display.setTextColor(ST7735_RED, ST7735_BLACK);
    
    char buf[32];
    sprintf(buf, "AQI:%d VOC:%d", aqi, tvoc);
    display.setCursor(2, 75); // Τέρμα κάτω
    display.print(buf);
}
