...
uint8_t LoRaClass::createCombinedPacket(double lat, double lon, uint32_t sats, uint8_t h, uint8_t m, uint8_t s,
                             float temp, float pressure, uint8_t aqi, uint16_t tvoc, uint16_t eco2, 
                             uint16_t uv, uint8_t* out, size_t outMax) const {
    ...
    // ENS160
    out[19] = aqi;
    uint16_t tvoc10 = tvoc;
    out[20] = (tvoc10 >> 8) & 0xFF;
    out[21] = tvoc10 & 0xFF;

    uint16_t eco2ppm = eco2;
    out[22] = (eco2ppm >> 8) & 0xFF;
    out[23] = eco2ppm & 0xFF;

    uint16_t uvScaled = (uint16_t)(uv * 10);
    out[24] = uvScaled >> 8;
    out[25] = uvScaled & 0xFF;
